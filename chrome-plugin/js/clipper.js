/*
* @author:ericzheng
* @email:2573578564@qq.com
* 逻辑:如果是触发了browser action或者是没有selection任何内容的情况下，如果点击保存网页
* 用extract-content库获得要抽取的内容用txt变量保存,如果是有selection,则获取selection的内容作为txt
* 获取到本网页的title,url，根据_id的用户id在mongodb数据库当中获取对应的用户，存到数据库当中
* 数据经过了encodeURI，在后台经过了mysql_escape_string处理
* */
/**
...
chrome.extension.onRequest.addListener(function(msg) {
  if(msg.action === "m_clipperPage" || msg.action === "m_clipperSelection"){
      var selection_result = rangy.getSelection().getRangeAt(0);
    //不管是哪一种，获取到了文本之后，右上角创建一个div
    upload_exec(selection_result);
  }else if(msg.action === "browser"){
      upload_exec('browser');
        //alert(2);
  }else if(msg.action === "m_clipperImage"){
        //alert(3);
  }
});

function upload_exec(txt) {
  //如果是https的,则直接返回
  if(document.location.protocol === "https:"){
    show_https();
    function show_https(){
      var p = document.createElement("div");
      p.setAttribute('id','dulishuo_info');
      p.style.width = "300px";
      p.style.height = "54px";
      p.style.background = "-webkit-gradient(linear,0% 0,0% 100%,from(#ca487f),to(#EEEEEE))";
      ...
      p.style.lineHeight = "54px";
      p.style.zIndex = "9999";
      p.innerHTML = '不支持https ^_^';
      document.body.appendChild(p);
      setTimeout(function(){
        //document.getElementById("dulishuo_info").style.display = 'none';
        document.body.removeChild(p);
      },2000);
    }
    return;
  }
  //判断用户有没有登录过,如果从来没有登录,则提供登录框 chrome.storage 存用户名 _id
  chrome.storage.local.get('dulishuo_user_string',function(obj){
    console.log('4785',obj);
    window.c = obj;
    //console.log('==');
    if(isEmpty(obj)){
      console.log('===> 4786');
     // alert(1);
      ...
      if(txt == ''){
        //alert('选取的内容是空的');
        //return;
        //走的逻辑和browser的逻辑是一样的，用工具选择出网页中的content，然后保存
        txt = (get_clip_content());
        //console.log(txt);
        //return;
      }
      var _id = JSON.parse(obj.dulishuo_user_string)._id;
      //把要保存的内容ajax上传到服务器,如果成功就显示保存成功
      var url = 'http://192.168.192.129/index.php/ceshi/upload_text?';
      var xmlhttp = new XMLHttpRequest;
      xmlhttp.onreadystatechange = function() {
        if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
          //console.log(xmlhttp.responseText);
          //return;
          //console.log(xmlhttp.responseText['code']);
      ...
      xmlhttp.send('{"_id":"'+_id+'","text":"'+encodeURI(txt)+'","url":"'+location.href+'","title":"'+document.title+'"}');
    }
    //console.log(JSON.parse(obj.dulishuo_user_string));
  });
  return false;
  //if ((document.readyState === "complete" || document.readyState == "loaded" || document.readyState == "interactive") && document.body) {
  //  //alert(802);
  //  if (commonTool.browser.isChrome && commonTool.isHTTPS) {
  //    var p = document.getElementById("h1");
  //    p.innerHTML = "该扩展暂不支持收藏https类网站";
  //    document.body.appendChild(p);
  //    //yApp.showError({
  //     // message: "Error: host site is under https protocol",
  //     // text: "该扩展暂不支持收藏https类网站"
  //    //});
  //    return
  //  }
  //  yApp.checkLogin(function(isLogin) {
  //    if (config.clipType === "m_clipperImage") {
  //      console.log("yApp checkLogin ClipperImage");
  //      window.clipperImage = new ClipperImage(config.clipOption);
  //      window.clipperImage.run();
  //      uiControl(yApp)
  //    } else {
  //      yApp.run();
  //      uiControl(yApp);
  //      if ((config.clipType === "m_clipperPage" || config.clipType === "m_clipperSelection") && isLogin) {
  //        if (!savingDiv) {
  //
  //
  //          createSavingDiv();
  ...
  //  })
  //} else {
  //  hLoop = setTimeout(upload_exec, 300)
  //}
};
...
function create_login_form(){
  var p = document.createElement("div");
  p.setAttribute('id','dulishuo_login');
  p.style.width = "292px";
  ...
  //p.innerHTML = txt;
  p.innerHTML = '<form id="dulishuo_login_form" class="dulishuo_login"><label>Username:</label><input type="text" name="username" id="dulishuo_name"><label>Password:</label><input type="password" name="password" id="dulishuo_password"><input type="submit" value="sign in" name="submit" class="submit" id="dulishuo_submit"></form><style>.dulishuo_login{border:1px solid #270644;width:250px;-moz-border-radius:20px;-webkit-border-radius:20px;background:-moz-linear-gradient(19% 75% 90deg,#4e0085,#963ad6);background:-webkit-gradient(linear,0 0,0 100%,from(#ca487f),to(#EEE));padding:20px;-moz-box-shadow:0 -5px 300px #270644;-webkit-box-shadow:0 -5px 300px #270644}.dulishuo_login label{font-size:12px;font-family:arial,sans-serif;list-style-type:none;color:#fff;text-shadow:#000 1px 1px;margin-bottom:10px;font-weight:700;letter-spacing:1px;text-transform:uppercase;display:block}.dulishuo_login input{-webkit-transition-property:-webkit-box-shadow,background;-webkit-transition-duration:.25s;padding:6px;border-bottom:0;border-left:0;border-right:0;border-top:1px solid #ad64e0;-moz-box-shadow:0 0 2px #000;-webkit-box-shadow:0 0 2px #000;margin-bottom:10px;background:#E9E9E9;width:230px}.dulishuo_login input.submit{-webkit-transition-property:-webkit-box-shadow,background;-webkit-transition-duration:.25s;width:100px;background:-webkit-gradient(linear,0 0,0 100%,from(#E6D2F4),to(#AFAEAE));background:-moz-linear-gradient(19% 75% 90deg,#781bb9,#963ad6);color:#fff;text-transform:uppercase;text-shadow:#000 1px 1px;border-top:1px solid #ad64e0;margin-top:10px}.dulishuo_login input.submit:hover{-webkit-box-shadow:0 0 2px #000;background:-webkit-gradient(linear,0 0,0 100%,from(#F2F2F2),to(#727E7B));background:-moz-linear-gradient(19% 75% 90deg,#781bb9,#963ad6)}.dulishuo_login input.submit:active{background:-webkit-gradient(linear,0 0,0 100%,from(#781bb9),to(#963ad6));background:-moz-linear-gradient(19% 75% 90deg,#963ad6,#781bb9)}.dulishuo_login input:hover{-webkit-box-shadow:0 0 4px #000;background:#FBF6FF}</style>';
  document.body.appendChild(p);

  //绑定事件
  var dulishuo_submit = document.getElementById("dulishuo_submit");
  var dulishuo_name = document.getElementById("dulishuo_name");
  var dulishuo_password = document.getElementById("dulishuo_password");
  dulishuo_submit.onclick = function(){
    //console.log(dulishuo_name.value);
    //console.log(dulishuo_password.value);

    //ajax验证账号密码 如果成功登录了之后,则提示登录成功,内容已保存
    var url = 'http://192.168.192.129/index.php/ceshi/login_check?val='+JSON.stringify({'n':dulishuo_name.value,'p':dulishuo_password.value});
    var xmlhttp = new XMLHttpRequest;
    xmlhttp.onreadystatechange = function() {
      if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
        //console.log(xmlhttp.responseText);
        //console.log(xmlhttp.responseText);
        //{'code':1,'dulishuo_id:123,'dulishuo_nickname':'eric'}
        var res = JSON.parse(xmlhttp.responseText);
        if(res.code == 1) {
          document.body.removeChild(document.getElementById("dulishuo_login"));
          chrome.storage.local.set({
            'dulishuo_user_string': JSON.stringify({
              'name': res.dulishuo_nickname,
              '_id': res.dulishuo_id
            })
          }, function () {
            //alert('存储成功');
          ...
          function show_login_error(){
            ...
            m.setAttribute('id','dulishuo_error');
            m.style.width = "300px";
            m.style.height = "54px";
            m.style.background = "-webkit-gradient(linear,0% 0,0% 100%,from(#ca487f),to(#EEEEEE))";
            ...
            m.style.lineHeight = "54px";
            m.style.zIndex = "9999";
            m.innerHTML = '登录失败,用户名或密码不对';
            document.body.appendChild(m);
            setTimeout(function(){
              document.body.removeChild(document.getElementById("dulishuo_error"));
            },2000);
          }
        }
      }
...
//获取网页上的内容 一般是抽取出来class=content的正文之类的
function get_clip_content(){
    ...
    //extract content
    var extract = extractContent(document);
    var extractedContent;
    if (extract.isSuccess) {
      extractedContent = extract.content.asNode();
      if (extractedContent.nodeType == 3) {
        extractedContent = extractedContent.parentNode;
      }
      //console.log(extractedContent);//抽取到了内容 <================
      //setTimeout(function() {
      ...
      //没有选择的时候选择整个网页
      extractedContent = document.body;
    ...
    return (new XMLSerializer()).serializeToString(extractedContent);
  }
...
function show_success(){
  ...
  setTimeout(function(){
    //document.getElementById("dulishuo_info").style.display = 'none';
    if(document.getElementById('dulishuo_info'))
    document.body.removeChild(document.getElementById('dulishuo_info'));
  },2000);
}